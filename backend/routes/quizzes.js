...
function generateFallbackQuiz(skill, chapterId, difficulty) {
  return {
    skill: skill,
...
router.get('/chapter/:chapterId', authenticateToken, async (req, res) => {
  try {
    ...
    const { skill, chapter, difficulty = 'medium', roadmapId } = req.query;
    ...
    console.log(`🧠 Generating roadmap-based quiz for: ${skill} - Chapter ${chapterId}`);

    // Try to get roadmap chapter data
    let chapterData = null;
    try {
      if (roadmapId) {
        // Fetch roadmap data to get actual chapter content
        ...
        if (roadmapResponse.ok) {
          ...
          if (roadmapResult.success && roadmapResult.data.chapters) {
            chapterData = roadmapResult.data.chapters.find(ch => ch.id == chapterId);
          }
        }
      }
    } catch (error) {
      console.log('Could not fetch roadmap data, using skill-based generation');
    }

    // Generate quiz with working Gemini service using chapter data
    let quizData;
    try {
      quizData = await workingGeminiService.generateChapterBasedQuiz(
        skill, 
        chapter || `Chapter ${chapterId}`,
        difficulty,
        chapterData
      ...
      quizData = generateFallbackQuiz(skill, chapterId, difficulty);
    }
