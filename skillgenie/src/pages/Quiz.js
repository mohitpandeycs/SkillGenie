...
const Quiz = () => {
  ...
  const getSkillAndChapter = () => {
    ...
    if (location.state?.skill && location.state?.chapter) {
      return {
        ...
        chapterData: location.state.chapterData,
        roadmapId: location.state.roadmapId
      };
    }
    ...
    if (savedRoadmap) {
      ...
      const currentChapter = roadmap.chapters?.[roadmap.currentChapter - 1];
      return {
        skill: roadmap.skill || 'JavaScript',
        chapter: roadmap.currentChapter || 1,
        chapterData: currentChapter,
        roadmapId: roadmap.id || 'default'
      };
    }
  ...
  const fetchQuizData = async () => {
    ...
    const { skill, chapter, chapterData, roadmapId } = getSkillAndChapter();
    
    try {
      // Build API URL with chapter-based parameters
      const apiUrl = new URL(`${getApiUrl('QUIZ')}/${chapter}`, window.location.origin);
      apiUrl.searchParams.append('skill', skill);
      if (roadmapId) {
        apiUrl.searchParams.append('roadmapId', roadmapId);
      }
      if (chapterData?.title) {
        apiUrl.searchParams.append('chapter', chapterData.title);
      }
      
      console.log(`🎯 Fetching chapter-based quiz: ${chapterData?.title || chapter} for ${skill}`);
